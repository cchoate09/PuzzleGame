const ACHIEVEMENTS = [
  {
    id: "first-stamp",
    title: "First Stamp",
    description: "Complete the opening room and officially join the route.",
  },
  {
    id: "side-route",
    title: "Side Route",
    description: "Solve an optional side room.",
  },
  {
    id: "festival-line",
    title: "Festival Line",
    description: "Reach the Rooftops and restore the final delivery lane.",
  },
  {
    id: "careful-hands",
    title: "Careful Hands",
    description: "Finish a room without using any hints.",
  },
];

const DISTRICTS = [
  {
    id: "mailroom",
    title: "Mailroom",
    subtitle: "The first tears in the paper",
    unlockPostmarks: 0,
    summary: "Learn how stitched sheets overlap and how to step through them cleanly.",
    journalTitle: "Mina's Starter Journal",
    journalBody:
      "The storm did not break the town so much as peel it apart. Mina says a good postkeeper does not force paper flat. They learn the folds, the little stubborn bulges, and work with them.",
  },
  {
    id: "market",
    title: "Market",
    subtitle: "Routes braided through stalls and awnings",
    unlockPostmarks: 1,
    summary: "Transfer parcels between layers and use them to hold doors open or reveal shortcuts.",
    journalTitle: "Market Sketches",
    journalBody:
      "The market vendors patched their awnings before their roofs. Priorities. Every route is now tied to every other route, so fixing one usually means fixing three.",
  },
  {
    id: "greenhouse",
    title: "Greenhouse",
    subtitle: "Lantern light and vine projections",
    unlockPostmarks: 2,
    summary: "Projection lanterns cast temporary bridges onto neighboring layers.",
    journalTitle: "Greenhouse Notes",
    journalBody:
      "Some paper is seeded with silver thread. Mina calls it greenhouse stock. Shine a lamp through it and the next layer blooms into shape.",
  },
  {
    id: "clocktower",
    title: "Clocktower",
    subtitle: "Timing made visible",
    unlockPostmarks: 3,
    summary: "Echo couriers repeat your previous move one beat later.",
    journalTitle: "Clocktower Timing",
    journalBody:
      "The tower's routes are delayed by one bell. If I move now, my echo steps a moment later. It is unsettling to solve puzzles with my own future in the room.",
  },
  {
    id: "theater",
    title: "Theater",
    subtitle: "Shadows on the backdrops",
    unlockPostmarks: 4,
    summary: "Shadow couriers mirror your movements and can latch hidden mechanisms.",
    journalTitle: "Theater Blocking",
    journalBody:
      "Every prop in the theater has a partner behind the curtain. Nothing moves alone, especially not under the footlights.",
  },
  {
    id: "rooftops",
    title: "Rooftops",
    subtitle: "The festival line",
    unlockPostmarks: 5,
    summary: "Mix switches, projection, and layer travel in one final route.",
    journalTitle: "Rooftop Draft",
    journalBody:
      "When the routes finally align, the whole town reads like one folded letter. Rooftops first, festival after.",
  },
  {
    id: "attic",
    title: "Attic",
    subtitle: "A hidden route tucked into the rafters",
    unlockPostmarks: 6,
    summary: "A secret bonus room for players who want one more twist.",
    journalTitle: "Attic Margin",
    journalBody:
      "There is always one route Mina forgets to mention aloud. She says every town deserves one secret line for the people who still look up.",
  },
];

const ROOM_DEFS = [
  {
    id: "mailroom-01",
    districtId: "mailroom",
    title: "Loose Thread",
    optional: false,
    unlockCost: 0,
    postmarks: 1,
    objective: "Reach the glowing mailbox.",
    blurb: "The first sheet has peeled away from the second.",
    intro: [
      {
        speaker: "Mina",
        text: "Stitch markers connect identical coordinates. Stand on one and flip the room over.",
      },
    ],
    achievementId: "first-stamp",
    hintTiers: [
      "You do not need every corridor on the first sheet. Look for the stitched square.",
      "The stitch in the top layer lines up with another stitch below it. Switch layers from there.",
      "Walk to the stitch on the front sheet, switch to the back sheet, then take the open route to the mailbox.",
    ],
    layers: [
      {
        id: "front",
        name: "Front Sheet",
        tiles: [
          "#######",
          "#..S..#",
          "#.###.#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "back",
        name: "Address Sheet",
        tiles: [
          "#######",
          "#..S..#",
          "#.###G#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 1, y: 1, facing: "right" },
    entities: [],
    switches: [],
    doors: [],
  },
  {
    id: "mailroom-02",
    districtId: "mailroom",
    title: "Stamped Twice",
    optional: false,
    unlockCost: 0,
    postmarks: 1,
    objective: "Transfer the parcel out of your path and reach the mailbox.",
    blurb: "The parcel is in the right place on the wrong sheet.",
    intro: [
      {
        speaker: "Mina",
        text: "Transfers move a parcel to the same coordinates on the next sheet. You only need the route clear for yourself.",
      },
    ],
    hintTiers: [
      "This parcel is not meant to be pushed down the hall.",
      "Stand next to the parcel and transfer it to the other layer instead of shoving it forward.",
      "Move to the tile left of the parcel, press transfer, then walk through the cleared lane to the mailbox.",
    ],
    layers: [
      {
        id: "front",
        name: "Front Sheet",
        tiles: [
          "#######",
          "#.....#",
          "#.###.#",
          "#...G.#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "back",
        name: "Back Sheet",
        tiles: [
          "#######",
          "#.....#",
          "#.###.#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 1, y: 3, facing: "right" },
    entities: [
      {
        id: "parcel-a",
        type: "parcel",
        layer: 0,
        x: 3,
        y: 3,
        pushable: true,
        solid: true,
      },
    ],
    switches: [],
    doors: [],
  },
  {
    id: "market-01",
    districtId: "market",
    title: "Counterweight",
    optional: false,
    unlockCost: 0,
    postmarks: 1,
    objective: "Leave a parcel on the plate, switch layers, and use the opened route.",
    blurb: "The market lifts its shutters with simple pressure plates.",
    intro: [
      {
        speaker: "Market Clerk",
        text: "A good parcel is sometimes more useful parked on a plate than delivered.",
      },
    ],
    hintTiers: [
      "The plate is meant to stay pressed while you move away from it.",
      "Push the parcel onto the plate first, then go use the stitch marker.",
      "Move the parcel onto the floor plate in the front sheet, walk to the stitch, switch to the back sheet, and use the now-open door.",
    ],
    layers: [
      {
        id: "awnings",
        name: "Awnings",
        tiles: [
          "#######",
          "#..S..#",
          "#.....#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "arcade",
        name: "Arcade",
        tiles: [
          "#######",
          "#..S..#",
          "#....G#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 5, y: 4, facing: "left" },
    entities: [
      {
        id: "parcel-b",
        type: "parcel",
        layer: 0,
        x: 2,
        y: 4,
        pushable: true,
        solid: true,
      },
    ],
    switches: [{ id: "market-plate", layer: 0, x: 1, y: 4 }],
    doors: [{ id: "market-door", layer: 1, x: 3, y: 2, switchIds: ["market-plate"] }],
  },
  {
    id: "market-side-01",
    districtId: "market",
    title: "Tied Parcel",
    optional: true,
    unlockCost: 0,
    postmarks: 1,
    objective: "Transfer the parcel onto the hidden plate to open the side route.",
    blurb: "An optional room that rewards reading both layers at once.",
    intro: [
      {
        speaker: "Mina",
        text: "Some side routes only ask whether you noticed the second layer at all.",
      },
    ],
    achievementId: "side-route",
    hintTiers: [
      "The door opens from a plate you cannot reach directly.",
      "The parcel already sits above the hidden switch. Try transferring it instead of walking it there.",
      "Move next to the parcel in the front sheet, transfer it to the back sheet, then return to the front route and walk through the opened door.",
    ],
    layers: [
      {
        id: "front",
        name: "Front Stall",
        tiles: [
          "#######",
          "#.....#",
          "#....G#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "back",
        name: "Back Stall",
        tiles: [
          "#######",
          "#.....#",
          "#.....#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 1, y: 2, facing: "right" },
    entities: [
      {
        id: "parcel-c",
        type: "parcel",
        layer: 0,
        x: 3,
        y: 3,
        pushable: true,
        solid: true,
      },
    ],
    switches: [{ id: "side-plate", layer: 1, x: 3, y: 3 }],
    doors: [{ id: "side-door", layer: 0, x: 3, y: 2, switchIds: ["side-plate"] }],
  },
  {
    id: "greenhouse-01",
    districtId: "greenhouse",
    title: "Paper Vines",
    optional: false,
    unlockCost: 0,
    postmarks: 1,
    objective: "Project a bridge onto the lower sheet and cross the gap.",
    blurb: "Lanterns draw temporary structure where the paper has torn away.",
    intro: [
      {
        speaker: "Gardener",
        text: "Line the lamp up with the tear. The next layer will grow a bridge where the light lands.",
      },
    ],
    hintTiers: [
      "The lamp affects the same coordinates in the other layer.",
      "The pit sits at the same x and y as the square where the lamp should stop.",
      "Push the lantern to the center lane at x3 y2 on the top sheet, switch layers, and walk across the projected bridge.",
    ],
    layers: [
      {
        id: "lantern-bed",
        name: "Lantern Bed",
        tiles: [
          "#######",
          "#..S..#",
          "#.....#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "vine-bed",
        name: "Vine Bed",
        tiles: [
          "#######",
          "#..S..#",
          "#..~.G#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 5, y: 4, facing: "left" },
    entities: [
      {
        id: "lantern-a",
        type: "projector",
        layer: 0,
        x: 2,
        y: 3,
        pushable: true,
        solid: true,
        projectionTargets: [{ layer: 1, dx: 0, dy: 0 }],
      },
    ],
    switches: [],
    doors: [],
  },
  {
    id: "clocktower-01",
    districtId: "clocktower",
    title: "One Bell Late",
    optional: false,
    unlockCost: 0,
    postmarks: 1,
    objective: "Use the echo courier to hold the door long enough to pass.",
    blurb: "Your last move returns as a delayed helper on another sheet.",
    intro: [
      {
        speaker: "Bell Keeper",
        text: "The echo courier repeats your previous move exactly one turn later. Give it a beat to catch up.",
      },
    ],
    hintTiers: [
      "The echo courier needs one turn before it copies your first move.",
      "Move once, wait once, then take advantage of the opened door while the echo stays on the switch.",
      "Walk right, wait, walk right through the opened door, then walk right again to the mailbox.",
    ],
    layers: [
      {
        id: "clock-face",
        name: "Clock Face",
        tiles: [
          "#######",
          "#.....#",
          "#...G.#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "inner-works",
        name: "Inner Works",
        tiles: [
          "#######",
          "#.....#",
          "#.....#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 1, y: 2, facing: "right" },
    entities: [
      {
        id: "echo-a",
        type: "echo",
        layer: 1,
        x: 1,
        y: 4,
        solid: true,
        pushable: false,
        echoDelay: 1,
        queuedAction: null,
      },
    ],
    switches: [{ id: "clock-plate", layer: 1, x: 2, y: 4 }],
    doors: [{ id: "clock-door", layer: 0, x: 3, y: 2, switchIds: ["clock-plate"] }],
  },
  {
    id: "theater-01",
    districtId: "theater",
    title: "Understudy",
    optional: false,
    unlockCost: 0,
    postmarks: 1,
    objective: "Let your shadow open the route while you pass through.",
    blurb: "The stage mirrors movement even when the audience cannot see it.",
    intro: [
      {
        speaker: "Stagehand",
        text: "Your shadow moves in the opposite direction across its own sheet. Think about where it lands, not where you do.",
      },
    ],
    hintTiers: [
      "One move to the right sends the shadow one move to the left.",
      "The shadow only needs to step on the switch once for you to get through the door.",
      "Move right to place the shadow on the switch, then continue right through the door before the route closes behind you.",
    ],
    layers: [
      {
        id: "stage",
        name: "Stage",
        tiles: [
          "#######",
          "#.....#",
          "#...G.#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "backdrop",
        name: "Backdrop",
        tiles: [
          "#######",
          "#.....#",
          "#.....#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 1, y: 2, facing: "right" },
    entities: [
      {
        id: "shadow-a",
        type: "shadow",
        layer: 1,
        x: 5,
        y: 4,
        solid: true,
        pushable: false,
        mirrorAxis: "vertical",
      },
    ],
    switches: [{ id: "stage-plate", layer: 1, x: 4, y: 4 }],
    doors: [{ id: "stage-door", layer: 0, x: 3, y: 2, switchIds: ["stage-plate"] }],
  },
  {
    id: "rooftops-01",
    districtId: "rooftops",
    title: "Festival Line",
    optional: false,
    unlockCost: 0,
    postmarks: 1,
    objective: "Hold the door, project the bridge, and restore the final delivery lane.",
    blurb: "This is the first true mixed-mechanic route in the prototype.",
    intro: [
      {
        speaker: "Mina",
        text: "By now the town expects more than one insight at a time. Hold the line open, then build the bridge.",
      },
    ],
    achievementId: "festival-line",
    hintTiers: [
      "One parcel belongs on the switch. The lantern belongs where the tear is.",
      "Park the parcel on the switch first so you do not have to revisit it after the bridge is ready.",
      "Push the parcel onto the floor plate, move the lantern to x3 y2, switch layers at the stitch, then walk over the bridge and through the open door to the mailbox.",
    ],
    layers: [
      {
        id: "roofline",
        name: "Roofline",
        tiles: [
          "#######",
          "#..S..#",
          "#.....#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "gutter",
        name: "Gutter Route",
        tiles: [
          "#######",
          "#..S..#",
          "#..~.G#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 5, y: 4, facing: "left" },
    entities: [
      {
        id: "parcel-d",
        type: "parcel",
        layer: 0,
        x: 2,
        y: 4,
        pushable: true,
        solid: true,
      },
      {
        id: "lantern-b",
        type: "projector",
        layer: 0,
        x: 2,
        y: 3,
        pushable: true,
        solid: true,
        projectionTargets: [{ layer: 1, dx: 0, dy: 0 }],
      },
    ],
    switches: [{ id: "roof-plate", layer: 0, x: 1, y: 4 }],
    doors: [{ id: "roof-door", layer: 1, x: 4, y: 2, switchIds: ["roof-plate"] }],
  },
  {
    id: "attic-01",
    districtId: "attic",
    title: "Dustline",
    optional: true,
    unlockCost: 0,
    postmarks: 1,
    objective: "Latch the hidden switch with your shadow, then climb into the rafters.",
    blurb: "A secret bonus room built around one permanent switch.",
    intro: [
      {
        speaker: "Mina",
        text: "You found the attic route. The old latch still works, if your shadow can reach it first.",
      },
    ],
    hintTiers: [
      "The switch only needs to be touched once.",
      "Use your first move to the right to send the shadow onto the latch, then focus on reaching the stitch.",
      "Move right to trigger the shadow latch, head for the stitch at the top, switch to the attic sheet, and follow the now-open route to the mailbox.",
    ],
    layers: [
      {
        id: "rafters-front",
        name: "Rafters Front",
        tiles: [
          "#######",
          "#.S...#",
          "#.....#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
      {
        id: "rafters-back",
        name: "Rafters Back",
        tiles: [
          "#######",
          "#.S...#",
          "#....G#",
          "#.....#",
          "#.....#",
          "#######",
        ],
      },
    ],
    start: { layer: 0, x: 1, y: 4, facing: "right" },
    entities: [
      {
        id: "shadow-b",
        type: "shadow",
        layer: 1,
        x: 5,
        y: 4,
        solid: true,
        pushable: false,
        mirrorAxis: "vertical",
      },
    ],
    switches: [{ id: "attic-latch", layer: 1, x: 4, y: 4, sticky: true }],
    doors: [{ id: "attic-door", layer: 1, x: 3, y: 2, switchIds: ["attic-latch"] }],
  },
];

const ROOM_LOOKUP = Object.fromEntries(ROOM_DEFS.map((room) => [room.id, room]));

export { ACHIEVEMENTS, DISTRICTS, ROOM_DEFS };

export function getRoomById(roomId) {
  return ROOM_LOOKUP[roomId];
}

export function getDistrictById(districtId) {
  return DISTRICTS.find((district) => district.id === districtId) || null;
}

export function getRoomsForDistrict(districtId) {
  return ROOM_DEFS.filter((room) => room.districtId === districtId);
}

export function buildCampaignIndex() {
  return {
    achievements: ACHIEVEMENTS,
    districts: DISTRICTS,
    rooms: ROOM_DEFS,
    roomsById: ROOM_LOOKUP,
  };
}
